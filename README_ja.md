...
**大きな Markdown を見出しレベルで分割し、通し番号付きインデックス（目次）とセクション配列の JSON に変換する。LLM がインデックスを読んで必要なセクションだけを取得する — ファイル全体をコンテキストに読み込む代わりに。**
...
md2idx がない場合、大きなドキュメントの1セクションを読むためにファイル全体を読み込むことになる:

```
Read spec.md              → 5,000 行すべてがコンテキストに入る
「リトライポリシーは？」    → エージェントが全体から該当箇所を探す
```

md2idx を使えば、エージェントはまず 20 行のインデックスを読み、必要な箇所だけ取得する:

```sh
npx md2idx spec.md | jq -r '.index'       # 約 20 行（目次）
npx md2idx spec.md | jq -r '.sections[5]'  # 約 80 行（そのセクションだけ）
# 合計: 5,000 行の代わりに約 100 行
```

### grep で見出しを取ればよいのでは？

`grep -nE '#{1,6} ' spec.md` で見出し一覧は取れる。単純なケースでは有効だが、md2idx は grep では解決できない問題をカバーする:

|                                              | grep                                             | md2idx                                     |
| -------------------------------------------- | ------------------------------------------------ | ------------------------------------------ |
| セクション本文の取得（見出し行だけでなく）   | 行範囲を計算し Read の offset/limit で取得が必要 | `jq '.sections[N]'`                        |
| setext 見出し（`===` / `---`）               | 検出不可                                         | 検出する                                   |
| コードフェンス内の `#`                       | 偽陽性                                           | スキップ                                   |
| 見出し内のインライン記法（`[link](url)` 等） | そのまま含まれる                                 | インデックスでは除去、セクション内では保持 |
