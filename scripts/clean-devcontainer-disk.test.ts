...
const setupMixedHomeFixture = (fixture: Fixture): { codexEntry: string; extEntry: string } => {
...
const setupHomeCategories = (fixture: Fixture): string[] => {
  ...
  const codexEntry = path.join(codexTmpDir(fixture), 'session-file')
  writeSizedFile(codexEntry, 1024)
  return [extEntry, cacacheChild, codexEntry]
}
...
describe('候補列挙 (extensionsCache / codex-tmp)', () => {
